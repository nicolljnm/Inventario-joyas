...
DATOS_FABRICA = [
    ...
    [15, "Brazallete (Acero)", 70, 38000, 25],
...
def grafico(df):
    ...
    df.plot(kind="bar", x="Nombre", y=["Cantidad", "Stock_Minimo"], ax=ax)
...
editado = st.data_editor(df, key="tabla", num_rows="dynamic")

if st.button("Guardar y Verificar Stock"):
    guardar_datos(editado)
    criticos = verificar_stock(editado)
    enviar_correo(criticos)
    if not criticos.empty:
        st.warning("Productos en nivel crítico:")
        st.dataframe(criticos)

if st.button("Ver gráfico de stock"):
    grafico(df)
